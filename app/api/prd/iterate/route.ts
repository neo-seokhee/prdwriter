...
import { generateIterationPRDPrompt } from '@/lib/anthropic/prompts';
import type { PRDIterationOutput } from '@/lib/anthropic/types';
import type { IteratePRDRequest, IteratePRDResponse, ErrorResponse } from '@/lib/types/api';

export async function POST(request: NextRequest) {
  try {
    const body: IteratePRDRequest = await request.json();
    const { product_id, new_insights } = body;

    if (!product_id || !new_insights) {
      const errorResponse: ErrorResponse = {
        error: 'product_id와 new_insights가 필요합니다',
      };
    ...
    const latestPRD = await getLatestPRDVersion(product_id);
    if (!latestPRD) {
      const errorResponse: ErrorResponse = {
        error: '기존 PRD를 찾을 수 없습니다. 먼저 초기 PRD를 생성하세요.',
      };
      return NextResponse.json(errorResponse, { status: 404 });
    }

    // Generate iteration prompt
    const prompt = generateIterationPRDPrompt(
      product,
      latestPRD.content,
      new_insights,
      latestPRD.version_number
    );
    ...
    let prdOutput: PRDIterationOutput;
    try {
      console.log('=== Claude 응답 디버깅 (Iterate) ===');
    ...
    const response: IteratePRDResponse = {
      prd_version_id: prdVersionId,
      version_number: versionNumber,
      content: prdOutput.prd_content,
      validation_markers: prdOutput.validation_markers,
      change_summary: prdOutput.change_summary,
    };

    return NextResponse.json(response, { status: 201 });
  } catch (error) {
    console.error('Error iterating PRD:', error);

    const errorResponse: ErrorResponse = {
      error: 'PRD 고도화에 실패했습니다',
